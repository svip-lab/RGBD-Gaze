...
import logging
import os
from models.gaze_aaai_is import resnet34, Decoder, DepthBCE, RefineDepth
...
class GazeTrainer(Trainer):
    def __init__(self,
        ...
        self.models.resnet = resnet34(pretrained=True)
        self.models.decoder = Decoder()
        self.models.depth_loss = DepthBCE(0)
        self.models.refine_depth = RefineDepth()
    ...
    def train_base(self, epochs, lr=1e-4, lambda_is=1.0, use_refined_depth=False, fine_tune_headpose=True):
        # prepare logger
        ...
        self.temps.lr = lr
        self.temps.lambda_is = lambda_is
    ...
    def _train_base_epoch(self):
        ...
        for i, batch in enumerate(self.temps.train_loader):
            ...
            if self.temps.fine_tune_headpose:
                head_pose, head_pose_feat, refined_depth = refine_depth(face_image, face_depth)
            else:
                with th.no_grad():
                    head_pose, head_pose_feat, refined_depth = refine_depth(face_image, face_depth)
            ...
            coord = decoder(lfeat, rfeat, head_pose_feat, left_eye_info, right_eye_info)
            loss_coord = F.mse_loss(coord, target)
            loss_is = F.mse_loss(head_pose, target)
            loss = loss_coord + (self.temps.lambda_is * loss_is if self.temps.fine_tune_headpose else 0.)
