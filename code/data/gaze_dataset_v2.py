...
class GazePointAllDataset(data.Dataset):
    def __init__(self, root_dir, w_screen=59.77, h_screen=33.62, transform=None, phase="train", **kwargs):
        ...
        self.kwargs = kwargs
        anno = pd.read_csv(os.path.join(root_dir, phase + "_filelist.csv"), index_col=0)
        if "landmark" in kwargs.keys():
            anno = anno[anno["has_landmark"]]
        # if os.path.isfile(os.path.join(root_dir, "depth_stat.pkl")):
        #     with open(os.path.join(root_dir, "depth_stat.pkl"), "rb") as fp:
        #         stat = pickle.load(fp)
        #         anno.drop(anno.index[stat[0]])
        root_dir = root_dir.rstrip("/").rstrip("\\")
        self.face_image_list = (root_dir + "/" + anno["face_image"]).tolist()
        self.face_depth_list = (root_dir + "/" + anno["face_depth"]).tolist()
        self.face_bbox_list = (root_dir + "/" + anno["face_bbox"]).tolist()
        self.le_image_list = (root_dir + "/" + anno["left_eye_image"]).tolist()
        self.re_image_list = (root_dir + "/" + anno["right_eye_image"]).tolist()
        self.le_depth_list = (root_dir + "/" + anno["left_eye_depth"]).tolist()
        self.re_depth_list = (root_dir + "/" + anno["right_eye_depth"]).tolist()
        self.le_bbox_list = (root_dir + "/" + anno["left_eye_bbox"]).tolist()
        self.re_bbox_list = (root_dir + "/" + anno["right_eye_bbox"]).tolist()
        self.le_coord_list = (root_dir + "/" + anno["left_eye_coord"]).tolist()
        self.re_coord_list = (root_dir + "/" + anno["right_eye_coord"]).tolist()
        self.gt_name_list = (root_dir + "/" + anno["gaze_point"]).tolist()
        if "landmark" in kwargs.keys():
            self.face_landmark = (root_dir + "/" + anno["face_landmark"]).tolist()

        for data_item in kwargs.keys():
            if data_item not in ("face_image", "face_depth", "eye_image", "eye_depth",
                                 "face_bbox", "eye_bbox", "gt", "eye_coord", "info", "landmark"):
    ...
    def __getitem__(self, idx):
        ...
        sample["index"] = th.FloatTensor([idx])
        ...
        if self.kwargs.get('landmark'):
            landmark = np.load(self.face_landmark[idx])
            sample['face_landmark'] = th.FloatTensor(landmark)
        ...
        if self.kwargs.get('info'):
            ...
            face_depth = np.clip((cv2.imread(self.face_depth_list[idx], -1) - 500) / 500, 0, 1.)
            # get info
            le_depth_ = le_depth[le_depth > 0]
            face_depth = face_depth[face_depth > 0]
            if len(le_depth_) > 0:
                le_info = [le_coor[0] / 1920, le_coor[1] / 1080, np.median(le_depth_)]
            else:
                le_info = [le_coor[0] / 1920, le_coor[1] / 1080, np.median(face_depth)]

            re_depth_ = re_depth[re_depth > 0]
            if len(re_depth_) > 0:
                re_info = [re_coor[0] / 1920, re_coor[1] / 1080, np.median(re_depth_)]
            else:
                re_info = [re_coor[0] / 1920, re_coor[1] / 1080, np.median(face_depth)]
