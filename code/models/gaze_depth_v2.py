...
class RGB2RelDepth(nn.Module):
    def __init__(self, num_landmark=68, dim_landmark_feat=64):
        super(RGB2RelDepth, self).__init__()
        self.encoder = resnet18(pretrained=True)
        self.num_landmark = num_landmark
        self.dim_embedding = dim_landmark_feat
        self.depth_embedding = nn.Sequential(
            nn.Linear(512, num_landmark * dim_landmark_feat, bias=False),
            nn.BatchNorm1d(num_landmark * dim_landmark_feat),
            nn.ReLU(inplace=True),
            nn.Linear(num_landmark * dim_landmark_feat, num_landmark * dim_landmark_feat, bias=True)
            # nn.Sigmoid()
        )
        self.depth = nn.Sequential(
            nn.Conv2d(dim_landmark_feat, 1, kernel_size=1, stride=1, padding=0, bias=False)
        )

    def forward(self, face_image, landmarks):
        bs = face_image.size(0)
        feat = self.encoder(face_image)
        depth_feat = self.depth_embedding(feat)
        d1 = depth_feat.view(bs, self.dim_embedding, self.num_landmark, 1).expand(bs, self.dim_embedding, self.num_landmark, self.num_landmark)
        d2 = depth_feat.view(bs, self.dim_embedding, 1, self.num_landmark).expand(bs, self.dim_embedding, self.num_landmark, self.num_landmark)
        depthdiff = self.depth(d1 - d2).squeeze(1)
        assert th.allclose(depthdiff, -depthdiff.transpose(1, 2)), depthdiff
        with th.no_grad():
